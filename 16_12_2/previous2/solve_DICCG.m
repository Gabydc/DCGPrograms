...
function solve_DICCG(nos,nli,e,iteration,dir,k)
...
[x2,iter2,e2,hl2,h2,h21]=ICCG(a,b,xi,iteration,tol,l,e,text1,nos,dir,def,fc+10);


%[x3,iter3,e3,hl3,h2,h21]=DICCG(a,b,xi,iteration,tol,zt,l,e,text1,nt,dir,def,fc);

[x4,iter4,e4,hl4,h2,h21]=DICCG(a,b,xi,iteration,tol,zli,l,e,text1,nli,dir,def,fc+10);
% 
 [x5,iter5,e5,hl5,h22,h222]=DICCG_1(a,b,xi,iteration,tol,zpod,l,e,text1,nli,dir,def,fc);
...
ylabel('log(||M^{-1}r^k||_2/||M^{-1}b||_2)','FontSize',17)
...
axis tight
 ...
 hold off
figure(fc)
%legend([hl2,hl4,hl5],'ICCG',['DICCG_' num2str(nli) '}' ],['DICCG_{POD}_' num2str(nli)])
ylabel('log(||M^{-1}r^k||_2/||M^{-1}b||_2)','FontSize',17)
xlabel('Iteration','FontSize',17)
set(gca,'fontsize',15)
 file='conv_def';
...
fprintf(fileID,'%6s %d %6s %6.2e %6s\n','ICCG &',iter2,' &',max(e2), ' \\');
%fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_{tot} &',iter3,' &',max(e3), ' \\');
fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_{li} &',iter4,' &',max(e4), ' \\');
...
% colorbar
figure(fc+2)
 h1d=plotingsolution(G,W,'ICCG',x2,1);
 %colorbar
 view(0,90)
 h1d=plotingsolution(G,W,'DICCG',x4,2);
 %colorbar
% view(45,30)
% h1d=plotingsolution(G,W,'DICCG_{tot}',x3,2);
 %colorbar
 figure(fc+1)
 view(1,90)
 h1=plotingsolution(G,W,'DICCG_{pod}',x5,2);
view(0,90)
