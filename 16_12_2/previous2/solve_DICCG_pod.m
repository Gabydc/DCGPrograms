...
function solve_DICCG_pod(nos,nt,nli,e,iteration,dir,k)
...
%   [V,D]=POD(x);
%  z(:,1)=x(:,5);
%[V,D,hpod]=defpodf_w(x,5);
[U,S,hpod1]=defpodf_D(x,dir);
    s=0;
     for i=1:nli
        s=s+1;
     zli(:,s)=x(:,i);
     end
zpod(:,1:nli)=U(:,nt-nli+1:nt);
fileID = fopen(text1,'w');
fprintf(fileID,'\n%6s %6s %6s %6s\n\n','Method ','& ', 'Cond',' \\');
fclose(fileID);
%[x1,iter1,e1,hl1,hl11,hl111,h2,h21]=CGF(a,b,xi,iteration,tol,e,text,nn,dir,def); 

[x2,iter2,e2,hl2,h2,h21]=ICCG(a,b,xi,iteration,tol,l,e,text1,nos,dir,def,fc);


[x3,iter3,e3,hl3,h2,h21]=DICCG(a,b,xi,iteration,tol,zt,l,e,text1,nt,dir,def,fc);

[x4,iter4,e4,hl4,h2,h21]=DICCG_11(a,b,xi,iteration,tol,zli,l,e,text1,nli,dir,def,fc);
% 
 [x5,iter5,e5,hl5,h2,h21]=DICCG_1(a,b,xi,iteration,tol,zpod,l,e,text1,nli,dir,def,fc);
%[x6,iter6,e6,hl6,h26,h216]=DICCG_2e(a,b,xi,iteration,tol,zp,l,e,text1,nn,dir,def,fc);
...
figure(fc)
legend([hl2,hl3,hl4,hl5],'ICCG',['DICCG_{' num2str(nt) '}'],['DICCG_{' num2str(nli) '}' ],['DICCG_{POD}'])
...
fprintf(fileID,'%6s %d %6s %6.2e %6s\n','ICCG &',iter2,' &',max(e2), ' \\');
fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_{15} &',iter3,' &',max(e3), ' \\');
fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_{4} &',iter4,' &',max(e4), ' \\');
 fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_{pod} &',iter5,' &',max(e5), ' \\');
%fprintf(fileID,'%6s %d %6s %6.2e %6s\n','DICCG_pod &',iter6,' &',max(e6), ' \\');
fclose(fileID);
...
% colorbar
% 
 h1d=plotingsolution_4(G,W,'DICCG_{4}',x4,2);
 %colorbar
 view(0,90)
 h1d=plotingsolution_4(G,W,'DICCG_{15}',x3,3);
 %colorbar
  view(0,90)
 h1d=plotingsolution_4(G,W,'DICCG_{pod}',x5,4);
 view(0,90)
 
 h1d=plotingsolution_4(G,W,'ICCG',x2,1);
 %colorbar
  view(0,90)
  figure(fc+2)
hsol=plotGrid(G, 'FaceAlpha', 0, 'EdgeAlpha', .1);
hsol=plotCellData(G,x5);
view(0,90)
axis equal tight; colormap(jet(128));
title('DICCG_{POD}','FontSize',16);
  file='solpod';
   B=[dir  file def '.fig'];
   saveas(hsol,B)
    B=[dir  file def '.jpg'];
...
% colorbar
% hed=plotingsolution(G,W,'error DICCGPOD',abs(a*x6-b),2);
% colorbar
% figure(fc+5)
% %h1=plotingsolution(G,W,'CG',x1,1);
...
% colorbar
% hdd=plotingsolution(G,W,'diff DICCGPOD',abs(x6-p),2);
% colorbar
