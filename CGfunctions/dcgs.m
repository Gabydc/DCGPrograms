function [px,flag,relres,iter,resvec] = dpcgs(A,b,Z,tol,maxit,M1,M2,x0,varargin)
...
n2b = norm(b);                     % Norm of deflated rhs vector, b
...
if ((nargin >= 6) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error(message('MATLAB:dcgs:WrongPrecondSize', m));
        end
    end
else
    existM1 = 0;
...
if ((nargin >= 8) && ~isempty(x0))
    if ~isequal(size(x0),[n,1])
        error(message('MATLAB:dcgs:WrongInitGuessSize', n));
    else
        x = x0;
    end
else
...
if ((nargin > 8) && strcmp(atype,'matrix') && ...
    ...
    error(message('MATLAB:dcgs:TooManyInputs'));
end
...
[Pr]=dvect(Z,EI,A,r);
r=Pr;
normr = norm(r);                   % Norm of deflated residual
normr_act = normr;
...
for ii = 1 : maxit
    ...
    resvec(ii+1) = normr;
             ...
             px=qvect(Z,EI,b)+tdvect(Z,EI,A,x);
